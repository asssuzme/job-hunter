...
export class CashfreeClientService {
  ...
  static async createPaymentAndRedirect(userEmail: string, userId: string, userName: string): Promise<void> {
    try {
      ...
      // Create a simple form submission to bypass server API calls
      const form = document.createElement('form');
      form.method = 'POST';
      form.action = 'https://api.cashfree.com/pg/orders';
      form.style.display = 'none';
      
      // Instead, let's use the Payment Link approach
      // This is a pre-created payment link that doesn't require API authentication
      const paymentLinkUrl = this.buildPaymentLink(userEmail, userId, userName, orderId);
